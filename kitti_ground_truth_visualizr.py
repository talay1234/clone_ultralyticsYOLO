...
class KITTIVisualizer:
        ...
        with open(label_file) as f:
            for line in f:
                ...
                label = {
                    ...
                    "alpha": float(parts[3]),
                    "bbox_2d": np.array([float(parts[4]), float(parts[5]), float(parts[6]), float(parts[7])]),
    ...
    def draw_3d_box(self, img, corners_2d, color, thickness=2, draw_face=True, colored_vertices=False):
        ...
        if colored_vertices:
            ...
            for i, corner in enumerate(corners_2d):
                ...
                cv2.putText(img, vertex_labels[i], label_pos, cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1)
        else:
    ...
    def draw_text(self, img, text, position, color, scale=0.5):
        ...
        # Draw background rectangle
        x, y = position
        cv2.rectangle(img, (x, y - text_height - 4), (x + text_width, y + baseline), (0, 0, 0), -1)
    ...
    def create_bird_eye_view(self, labels, img_width=600, img_height=600, x_range=(-20, 20), z_range=(0, 40)):
        ...
        # Add text
        cv2.putText(img, "Bird's Eye View", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
        cv2.putText(img, f"X: {x_range[0]}m to {x_range[1]}m", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
        cv2.putText(img, f"Z: {z_range[0]}m to {z_range[1]}m", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

        return img

    def create_3d_visualization(self, labels, figsize=(12, 10), x_range=(-30, 30), y_range=(-5, 5), z_range=(0, 80)):
        ...
        for label in labels:
            ...
            for face in faces:
                ...
                poly = Poly3DCollection([vertices], alpha=0.2, facecolor=color_rgb, edgecolor=color_rgb, linewidths=1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ground Truth KITTI 3D Bounding Box Visualizer")
    parser.add_argument("--data-dir", type=str, required=True, help="KITTI dataset directory")
    parser.add_argument("--sample", type=str, default="000001", help="Sample ID to visualize")
    parser.add_argument("--split", type=str, default="training", choices=["training", "testing"], help="Dataset split")
    parser.add_argument("--output", type=str, default=None, help="Output file path")
    parser.add_argument("--show", action="store_true", help="Display visualization")
    parser.add_argument(
        "--camera", type=str, default="left", choices=["left", "right", "both"], help="Which camera view to visualize"
    )
    parser.add_argument("--bev", action="store_true", help="Also create bird eye view")
    parser.add_argument("--3d", dest="vis_3d", action="store_true", help="Create interactive 3D visualization")
    parser.add_argument("--colored-vertices", action="store_true", help="Draw each 3D box vertex with different color")
    ...
    results = visualizer.visualize(
        ...
        draw_text_labels=True,
        camera=args.camera,
        colored_vertices=args.colored_vertices,
    )
