...
def select_dataset_subset(
    ...
    This function implements stratified sampling by object count to ensure the subset is representative of the full
    dataset.
    ...
    LOGGER.info(
        f"Selected {len(selected)} images for profiling (stratified by object count: {len(object_counts)} bins)"
    )
...
def run_baseline_profiling(
    ...
    Path(data_cfg.get("path", ".")).resolve()
...
def _write_bottleneck_analysis(path: Path, report: dict[str, Any]):
    ...
    with open(path, "w") as f:
        ...
        f.write(
            f"- **Total Validation Time**: {total_time.get('mean', 0.0):.2f}s ± {total_time.get('std', 0.0):.2f}s\n"
        )
